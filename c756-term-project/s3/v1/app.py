...
def delete_playlist(playlist_id):
    ...
    try:
        response = requests.delete(
            ...
            params={"objtype": "playlist", "objkey": playlist_id},
            headers={'Authorization': headers['Authorization']})
        return Response(json.dumps({"Message" : "Playlist deleted successfully!"}), 
                                    status=200, 
                                    mimetype='application/json')
    except Exception:
        return Response(json.dumps({"Message": "Error while Deleting playlist!"}),
                                    status=500, 
                                    mimetype='application/json')
